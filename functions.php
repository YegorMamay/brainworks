            ...
            <tr class="form-field term-group-wrap">
                ...
                <td>
                    <div class="term__image__wrapper">
                        ...
                        <input type="button" class="button button-one termeta_img_remove"
...
/* WOO: Перемещает закончивщиеся товары в конец START */
function bw_woocommerce_stock_status_order_init()
{
    if (class_exists('WooCommerce')) {
        add_filter('posts_clauses', 'order_by_stock_status', 2000);
    }
}
add_action('init', 'bw_woocommerce_stock_status_order_init');
...
// Removed: The quantity buttons are now handled globally via JS (brainworks.js) to ensure identical UI on Cart and Product pages. // Добавляет кнопки + и - END

//Перемещает описание категории под товары
function bw_woocommerce_reorder_archive_description()
{
    if (class_exists('WooCommerce')) {
...
add_action('init', 'bw_woocommerce_reorder_archive_description');
// end
...
function product_viewers_shortcode()
{
    ...
    <div class="product-viewers mrgn-bot-20 mrgn-top-20">
        ...
        <?php printf(esc_html__('%s people are viewing this right now', 'brainworks'), '<span id="viewers-count">' . rand(5, 20) . '</span>'); ?>
    </div>
...
// Делаем текст "View Cart" переводимым
add_filter('woocommerce_add_to_cart_params', 'brainworks_change_view_cart_text', 10, 1);
add_filter('woocommerce_get_script_data', 'brainworks_change_view_cart_text', 10, 2);
function brainworks_change_view_cart_text($params, $handle = '')
{
    // Если это вызов из woocommerce_get_script_data, проверяем handle
    if ($handle !== '' && $handle !== 'wc-add-to-cart') {
        return $params;
    }

    // Рекомендуемый вариант: использовать стандартный перевод WooCommerce.
    // Это автоматически подхватит перевод на RU/UA из плагина WooCommerce.
    $params['i18n_view_cart'] = esc_attr__('View cart', 'woocommerce');

    // Если вы все же хотите переводить через тему (Loco Translate), раскомментируйте строку ниже
    // и закомментируйте строку выше. После этого нажмите "Sync" (Синхронизировать) в Loco Translate.
    // $params['i18n_view_cart'] = esc_attr__('View Cart', 'brainworks');
...
function showhide_shortcode($atts, $content = null)
{
    ...
    $attributes = shortcode_atts(array(
        'type' => 'pressrelease',
        'more_text' => __('Show Press Release (%s More Words)', 'brainworks'),
        'less_text' => __('Hide Press Release (%s Less Words)', 'brainworks'),
        'hidden' => 'yes'
    ), $atts);
